...
<span ng-repeat="actual in $root.events |filter:logFilterFunction|orderBy:eventTimestampAsFloat"><span class="{{$root.config.EVENT_CLASS[actual .eventType]}}" style="word-wrap: break-word">{{actual.customTimeStamp}} {{actual.stackName}} - {{$root.config.EVENT_TYPE[actual .eventType]}}: {{actual.eventMessage}} - {{actual.statusType}} - {{actual.detailedStackStatus}}</span><br/></span>
                                                </pre>
                                            </div>
                                        </div>
                                    </form>
